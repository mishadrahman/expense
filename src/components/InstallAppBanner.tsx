...
import { X, Zap, WifiOff } from 'lucide-react';
import { InstallButton } from './InstallButton';

export const InstallAppBanner: React.FC = () => {
  const { isInstalled, isStandalone, dismissInstallBanner, isBannerDismissed } = usePWA();
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-300 flex items-center gap-3 mt-0.5">
              <span className="flex items-center gap-1">
                <Zap className="w-3 h-3 text-amber-400 inline" /> Native Mobile Feel
              </span>
              <span className="flex items-center gap-1">
                <WifiOff className="w-3 h-3 text-teal-400 inline" /> Offline Mode
              </span>
            </p>
          </div>
        </div>
